...
export default function FindGroupsPage({
  ...
  const handleJoinGroup = async (groupId: string) => {
    ...
    if (!isSelfSelectOpen()) {
      toast({
        title: "Hết thời hạn",
        description: "Thời hạn tự chọn nhóm đã kết thúc.",
      });
      return;
    }
    if (user.groupId) {
      toast({
        ...
        description: "Không thể tham gia nhóm khác.",
      });
      return;
    }
    ...
    if (g.memberCount >= g.maxMembers) {
      toast({
        title: "Nhóm đã đủ",
        description: "Nhóm này đã đủ thành viên. Vui lòng chọn nhóm khác.",
      });
      return;
    }
    try {
      ...
      toast({
        ...
        description: `Bạn đã tham gia ${updated.groupName}.`,
      });
  ...
  const handleApplyToGroup = async (groupId: string) => {
    toast({
      ...
      description: "Tính năng nộp đơn sẽ sớm có mặt.",
    });
  };
