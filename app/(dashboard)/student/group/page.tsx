...
import { Filter, Loader2, Crown, Sparkles } from "lucide-react";
...
import ChangeMockData, {
  type ChangeMockDataProps,
} from "@/components/features/ChangeMockData";
        ...
        try {
          ...
          const mappedGroups = groupsArray.map((g: any) => {
            ...
            return {
              ...
              courseCode:
                g.course?.courseCode || g.courseCode || selectedCourse,
  ...
  const handleJoinGroup = async (groupId: string) => {
    ...
    try {
      if (useMock) {
        ...
        toast({
          title: isFirstMember
            ? "🎉 Chúc mừng Tân Trưởng Nhóm!"
            : "Tham gia thành công (Mock)",
          description: isFirstMember
            ? "Bạn là thành viên đầu tiên và đã trở thành Leader."
            : `Bạn đã tham gia ${g.groupName}.`,
          className: isFirstMember
        ...
        if (!isGuid && user.email) {
          ...
          try {
            ...
            if (res.ok) {
              ...
              ok = true;
              console.log(
                "✅ [handleJoinGroup] Lấy userId từ API:",
                userIdToUse
              );
            }
            if (!ok) {
            ...
            console.warn(
              "❌ [handleJoinGroup] Lỗi lấy userId từ API:",
              apiError
            );
          }
        }
          ...
          <div className="flex gap-3 items-center">
            ...
            <div className="flex items-center gap-2">
              <Switch
                ...
                id="only-empty"
              />
              <label htmlFor="only-empty" className="text-sm text-gray-700">
                Chỉ hiện nhóm trống
              </label>
            </div>
