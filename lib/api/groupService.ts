...
const mapApiGroupToFeGroup = (g: any): FeGroup => {
  ...
  let feMembers: GroupMember[] = rawMembers.map((gm: any) => {
    ...
    // Try multiple paths to get fullName, similar to studentsWithoutGroup structure
    const fullName =
      gm.userProfileViewModel?.fullName ||
      student?.userProfileViewModel?.fullName ||
      (student ? getUserFullName(student) : null) ||
      gm.fullName ||
      gm.username ||
      gm.email ||
      "Thành viên";
  ...
  static async joinGroup(groupId: string, userId: string): Promise<FeGroup> {
    try {
      ...
      if (!isGuid && /@/.test(resolvedUserId)) {
        try {
          const u = await UserService.getApiUserEmail({
      ...
      if (!isGuid)
        throw new Error(
          "userId không hợp lệ (phải là GUID). Hãy truyền 'user.id' hoặc email để tự động chuyển đổi."
        );
